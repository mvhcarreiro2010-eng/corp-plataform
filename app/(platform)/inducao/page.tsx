...
import { CheckCircle, Lock, Play, Star, Loader2, X, ChevronRight, Trophy } from 'lucide-react'
...
export default function InducaoPage() {
  ...
  const completedSteps = trail.steps.filter((s) => s.progress[0]?.completed).length
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {/* Mapa de trilha estilo Duolingo */}
      <div className="relative">
        ...
        <div className="space-y-6 relative z-10">
          {trail.steps.map((step, i) => {
            ...
            const isActive = !isDone && (i === 0 || trail.steps[i - 1].progress[0]?.completed)
              ...
              <div key={step.id} className="flex flex-col items-center gap-3">
                ...
                >
                  {isDone ? (
                    ...
                    <span>
                      {step.type === 'QUIZ' ? '🎯' : step.type === 'VIDEO' ? '🎬' : step.type === 'TASK' ? '✏️' : '📖'}
                    </span>
                  ) : (
                ...
                {/* Label */}
                <div className={cn(
                  'text-center px-4 py-2 rounded-xl max-w-[220px]',
                  ...
                  <div className="flex items-center justify-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-amber-600">+{step.xpReward} XP</span>
                    {isActive && <Badge variant="default" className="text-xs ml-1 py-0">Agora</Badge>}
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-amber-600 font-medium flex items-center gap-1">
                    <Star className="w-3 h-3" />+{activeStep.xpReward} XP
                  </span>
                </div>
              </div>
              <button
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          quizAnswer === null
                            ...
                            : idx === activeStep.quizzes[0].answer
              ...
              {showXP && (
                <div className="flex items-center justify-center py-4">
                  <div className="bg-amber-100 text-amber-700 font-bold text-2xl px-6 py-3 rounded-2xl animate-bounce">
                    +{activeStep.xpReward} XP! ⭐
                  </div>
                </div>
              )}

              {/* Botão de concluir (não quiz) */}
              {activeStep.type !== 'QUIZ' && (
                <Button
                  onClick={() => completeStep()}
                  disabled={completing || activeStep.progress[0]?.completed || showXP}
                  className="w-full"
                >
                  {completing ? (
                    <><Loader2 className="w-4 h-4 animate-spin" /> Salvando...</>
                  ) : activeStep.progress[0]?.completed ? (
                    <><CheckCircle className="w-4 h-4" /> Já concluída!</>
                  ) : (
                    <>Concluir e ganhar +{activeStep.xpReward} XP <ChevronRight className="w-4 h-4" /></>
                  )}
                </Button>
              )}
            </div>
